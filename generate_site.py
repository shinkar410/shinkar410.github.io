...
def thumb(src):
    """tiny grid thumbnail (~560px) generated by make_thumbs.py; else fall back.
    Grid cells display small, so this avoids shipping the full-res source."""
    dec = urllib.parse.unquote(src)
    stem = re.sub(r'-\d+x\d+(?=\.\w+$)', '', dec)
    t = re.sub(r'\.\w+$', '-thumb.webp', stem)
    if (DOCS / t).exists():
        return urllib.parse.quote(t)
    return variant(src, 768)
...
def img_grid(srcs, rel, cls="g-imgs"):
    cells = []
    for s in srcs:
        cells.append(f'<a href="{rel}{u(full(s))}"><img loading="lazy" src="{rel}{u(thumb(s))}" alt=""></a>')
